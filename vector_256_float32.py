...
collection_name = "vector_256_float32"  # Updated collection name

# Create the collection if it doesn't exist
if not qdrant_client.collection_exists(collection_name):
    qdrant_client.create_collection(
        collection_name=collection_name,
        vectors_config=VectorParams(size=256, distance=Distance.COSINE),  # Update vector size to 256
    )
...
# Reduce or pad vector size to 256
def get_256_dim_vector(text):
    original_vector = embedding_model.encode(text, convert_to_numpy=True)
    if len(original_vector) >= 256:
        return original_vector[:256].astype(np.float32)  # Truncate to 256 dimensions
    else:
        # Pad with zeros if vector is smaller than 256
        padded_vector = np.zeros(256, dtype=np.float32)
...
for i, row in cleaned_data.iterrows():
    if pd.notna(row['combined']):
        vector = get_256_dim_vector(row['combined'])
