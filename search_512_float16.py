...
# Collection name
collection_name = "vector_512_float16"  # Update collection name to reflect new size

# Initialize embedding model
embedding_model = SentenceTransformer('all-MiniLM-L6-v2')  # Outputs 384-dimensional vectors

# Generate a 512-dimensional vector from text
def get_512_dim_vector(text):
    """
    Generate a 512-dimensional vector by truncating or padding the original vector.
    """
    original_vector = embedding_model.encode(text, convert_to_numpy=True)
    if len(original_vector) >= 512:
        return original_vector[:512].astype(np.float16)  # Truncate to 512 dimensions
    else:
        # Pad with zeros if the vector is smaller than 512
        padded_vector = np.zeros(512, dtype=np.float16)
...
def search(query, top_n=5):
    ...
    """
    query_vector = get_512_dim_vector(query)
